...
import { GET_CHAT_BY_ID, SUBSCRIBE_TO_MESSAGES, INSERT_MESSAGE, SEND_MESSAGE_TO_BOT } from '../graphql/queries'
...
export function ChatView() {
  ...
  const { data: messagesData, loading: messagesLoading, error: subscriptionError } = useSubscription(SUBSCRIBE_TO_MESSAGES, {
    variables: { chatId },
    skip: !chatId
  })

  // Insert message mutation
  const [insertMessage, { loading: sendingMessage }] = useMutation(INSERT_MESSAGE, {
    onError: (error) => {
  ...
  const [sendMessageToBot, { loading: sendingToBot }] = useMutation(SEND_MESSAGE_TO_BOT, {
    update: (cache, { data }) => {
      if (data?.sendMessage) {
        // The bot response will be automatically added via subscription
        // but we can also update the cache directly for immediate display
        ...
        if (existingMessages) {
          cache.writeQuery({
            ...
            variables: { chatId },
            data: {
              messages: [...existingMessages.messages, data.sendMessage]
            }
          });
        }
      }
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Then, send message to bot to get response
          ...
          <div className="bg-white/80 backdrop-blur-sm rounded-xl border border-gray-200 h-full flex flex-col">
                  ...
                  <div key={date}>
                    ...
                    <div className="space-y-4">
                      {dateMessages.map((message) => {
                        const isOwnMessage = message.user_id === user?.id && !message.is_bot
            ...
            <div className="border-t border-gray-200 p-4">
              <form onSubmit={handleSendMessage} className="flex items-end space-x-3">
                ...
                <button
                  type="submit"
                  disabled={!newMessage.trim() || sendingMessage || sendingToBot || isTyping}
              ...
              {(sendingMessage || sendingToBot) && (
                <div className="mt-2 text-xs text-gray-500 text-center">
                  {sendingMessage ? 'Sending message...' : 'Getting AI response...'}
                </div>
              )}
              {subscriptionError && (
                <div className="mt-2 text-xs text-red-500 text-center">
                  Connection issue - messages may not update in real-time
                </div>
              )}
            </div>
          </div>
